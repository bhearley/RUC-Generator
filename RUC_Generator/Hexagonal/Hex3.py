def Hex3(NB, R, F, M, VI=None, RI=None, I=None):
    """
    ...
    if nx % 2 != 0:
        nx -= 1
    ny = 2 * round((np.sqrt(3) * nx) / 2)  # nearest integer for hex aspect ratio
    ...
    if VI is not None:
        A_total = nx * ny
        A_interface = VI * A_total / len(centers)  # distribute over fibers
